    ...
    for (const repo of group) {
      ...
      if (fs.existsSync(repoPath)) {
        ...
        if (gitResult.status === 0) {
          ...
          console.log(chalk.yellow(`  ${repo}: Directory exists but is not a git repository. Skipping.`));
        }
        continue;
      }
      ...
      (err: any) => {
        ...
        if (Array.isArray(err)) {
          err.forEach((fail) => {
            ...
            console.error(
              chalk.yellow(`  Repository ${name} failed to clone (cwd: ${cwd}) with exit code ${exitCode}`)
            );
          });
